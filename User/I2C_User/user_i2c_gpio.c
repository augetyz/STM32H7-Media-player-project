...
uint8_t User_I2C_Mem_Read(uint16_t DevAddress, uint16_t MemAddress,uint8_t *pData, uint16_t Size)
{
    ...
    /*发送地址*/
    i2c_SendByte(DevAddress|I2C_WR);/*发送写命令*/
    if(i2c_WaitAck()==1)	/* 检测设备的ACK应答 */
    {
        return 0;
    }
    /*判断MemAddress地址位数，主要分辨16位地址和8位地址*/
    if((MemAddress&0XFF)==MemAddress)/*判断为八位寄存器地址*/
    {
    ...
    /*起始信号*/
    i2c_Start();
    i2c_SendByte(DevAddress|I2C_RD);/*发送读命令*/
    if(i2c_WaitAck()==1)	/* 检测设备的ACK应答 */
    {
        return 0;
    }
