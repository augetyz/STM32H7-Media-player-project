...
uint32_t SD_ReadBuffer[512];	//	读数据数组
...
void FatFs_Check(void)	//判断FatFs是否挂载成功，若没有创建FatFs则格式化SD卡
{
    BYTE work[_MAX_SS];
    uint8_t b=0;
    b=FATFS_LinkDriver(&SD_Driver, SDPath);		// 初始化驱动
...
int NumOf_Blocks=64;
static uint8_t SD_Test_ReadBuffer[512];	//	读数据数�???
uint8_t SD_Test(void)
{
    uint32_t i = 0;
    uint32_t ExecutionTime_Begin;		// �???始时�???
...
// 读取 >>>>>>>
    ...
    SD_Status = HAL_SD_ReadBlocks(&hsd1,SD_Test_ReadBuffer, 0, NumOf_Blocks,NumOf_Blocks*100);	//块读�???
    ...
    ExecutionSpeed = (float)BLOCKSIZE * NumOf_Blocks / ExecutionTime / 1024 ; 	// 计算读取速度，单�??? MB/S
